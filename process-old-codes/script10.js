...
function draw1 (err, rows, types, swimmers) {
    ...
    function drawCompetitions(d){
        ...
        function mouseLeaveMiniMenu (d){d3.selectAll(".swimrecords").style("stroke",function(d){return scaleColor2(d.sex)}).style("stroke-width","0.5px").style("opacity",0.5)}
        ...
        function drawEvents (d){
           ...
           var years = []
            ...
            for (k = +formatYear(dateExtent[0]) ; k < formatYear(dateExtent[1]); k++) {
                years.push(k)
            }

            //var years = d3.timeYear.range(dateExtent[0], dateExtent[1], 1);
            console.log(years);
            var listofvalues = []
            var datatemplate= d3.map()
            years.forEach(function(eachyear){

                var filtered = d.filter(function(row){
                    return row.year == eachyear
                })
                listofvalues.push(filtered)
            })
            console.log("filtered", listofvalues)
            var radius = 1024/5,
                speedSlower = d3.scaleSqrt().domain(speedExtent).range([1000,5000]),
            ...
            sketchingEvents = sketchingEvents
                .enter()
                .append("g")
                .attr("class", "event").style("opacity",1)

            sketchingEvents.each(
                ...
                function(e,j){
                    ...
                    //if (e.length > 0){
                    var groupcircle = d3.select(this).selectAll("circle").data(e)

                        var groupcircleenter = groupcircle.enter().append("circle")
                        .attr("class", function(d,i){

                                //var yr = d.year
                                //console.log(d3.select("#year"),yr )
                                d3.select("#year")
                                    .html(i)
                                    .style("padding-left",(radius*2-49/2)+"px")
                                    .style("margin-top",-(500)+"px");
                        ...
                        .delay(function(d,i){return speedByTime3(d.date)})
                        .on("start",function(d){
                                d3.select("#year")
                                    .html(d.year)
                                    .style("padding-left",(radius*2-49/2)+"px")
                                    .style("margin-top",-(500)+"px");
                                d3.select(this).style("opacity",0).attr("r",function(d){
